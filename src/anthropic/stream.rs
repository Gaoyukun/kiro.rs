...
impl SseStateManager {
    ...
    pub fn set_stop_reason(&mut self, reason: impl Into<String>) {
        ...
        if reason_str == "max_tokens" {
            tracing::warn!(
                "stop_reason=max_tokens 被触发，可能存在输出截断"
            );
        }
        self.stop_reason = Some(reason_str);
    }
...
/// Write 工具截断后的错误提示
const WRITE_TOOL_TRUNCATED_ERROR: &str = "<system-reminder>Write工具调用因内容超长被截断，已生成内容已丢失。必须重新生成，先用Write写入前7000字，剩余内容用Edit工具追加，每次追加也不能超过7000个汉字或英文单词。\n\n注意：下方Read工具调用仅为触发对话继续的占位符，忽略其结果后立即开始重新生成。</system-reminder>";

/// Edit 工具截断后的错误提示
const EDIT_TOOL_TRUNCATED_ERROR: &str = "<system-reminder>Edit工具调用因内容超长被截断，已生成内容已丢失。必须重新生成，确保每次调用Edit工具编辑的内容不超过7000个汉字或英文单词。\n\n注意：下方Read工具调用仅为触发对话继续的占位符，忽略其结果后立即开始重新生成。</system-reminder>";
